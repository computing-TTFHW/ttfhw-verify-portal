      ...
      <table className="w-full min-w-[1300px] table-fixed">
        <thead>
          <tr className="border-b border-slate-200 bg-slate-50">
            ...
            <th className="w-[5%] px-3 py-3 text-center text-xs font-semibold text-slate-600 xl:px-4 xl:text-sm whitespace-nowrap">静态检查</th>
            <th className="w-[7%] px-3 py-3 text-center text-xs font-semibold text-slate-600 xl:px-4 xl:text-sm whitespace-nowrap">Devcontainer</th>
            <th className="w-[8%] px-3 py-3 text-right text-xs font-semibold text-slate-600 xl:px-4 xl:text-sm whitespace-nowrap">生成时间</th>
          </tr>
        </thead>
        <tbody className="divide-y divide-slate-100">
          {repos.map(repo => (
            <tr key={repo.name} className="transition-colors hover:bg-slate-50">
              <td className="px-3 py-3 xl:px-4">
                <div className="flex items-center gap-2 min-w-0">
                  <Link
                    to={`/repo/${encodeURIComponent(repo.name)}`}
                    className="font-medium text-blue-600 break-words hover:text-blue-800 hover:underline"
                  >
                    {repo.displayName}
                  </Link>
                  {repo.url && (
              ...
              <td className="px-3 py-3 text-right text-xs xl:px-4 xl:text-sm">
                {repo.testTotal && repo.testTotal > 0
                  ? (
                    <span className={repo.testFailed === 0 ? 'text-sky-400' : 'text-purple-400'}>
              ...
              <td className="px-3 py-3 text-center text-xs text-slate-600 xl:px-4 xl:text-sm">
                {repo.hasStaticAnalysis
                  ? <span className="text-sky-400 font-medium">有</span>
                  : <span className="text-slate-500">无</span>}
              </td>
              <td className="px-3 py-3 text-center text-xs text-slate-600 xl:px-4 xl:text-sm">
                {repo.hasDevcontainer
                  ? <span className="text-sky-400 font-medium">有</span>
